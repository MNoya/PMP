function MassTeleportStart( event )
    ...
    local nearby_units = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, radius, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NOT_MAGIC_IMMUNE_ALLIES, FIND_CLOSEST, false)
    ...
    for k,unit in pairs(nearby_units) do
        if unit:GetOwner() == owner and not unit.bTeleporting and (#ability.teleport_units <= max_units_teleported) then
